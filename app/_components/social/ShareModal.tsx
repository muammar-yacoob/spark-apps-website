...
const PLATFORMS = [
  { name: 'X', key: 'twitter', color: '#000000' },
  { name: 'Bluesky', key: 'bluesky', color: '#0085FF' },
  { name: 'Threads', key: 'threads', color: '#000000' },
...
const ICONS: Record<string, string> = {
  ...
  bluesky:
    '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M5.202 2.857C7.954 4.922 10.913 9.11 12 11.358c1.087-2.247 4.046-6.436 6.798-8.501C20.783 1.366 24 .213 24 3.883c0 .732-.42 6.156-.667 7.037-.856 3.061-3.978 3.842-6.755 3.37 4.854.826 6.089 3.562 3.422 6.299-5.065 5.196-7.28-1.304-7.847-2.97-.104-.305-.152-.448-.153-.327 0-.121-.05.022-.153.327-.568 1.666-2.782 8.166-7.847 2.97-2.667-2.737-1.432-5.473 3.422-6.3-2.777.473-5.899-.308-6.755-3.369C.42 10.04 0 4.615 0 3.883c0-3.67 3.217-2.517 5.202-1.026"/></svg>',
  threads:
...
function buildLinks(text: string, url: string) {
  const t = encodeURIComponent(text);
  const u = encodeURIComponent(url);
  const full = encodeURIComponent(`${text}\n${url}`);
  return {
    twitter: `https://twitter.com/intent/tweet?text=${t}&url=${u}`,
    bluesky: `https://bsky.app/intent/compose?text=${full}`,
    threads: `https://www.threads.net/intent/post?text=${full}`,
...
export function ShareModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const links = buildLinks(tagline, url);
  ...
  function handleSocialClick(href: string) {
  ...
  return (
    ...
    >
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: inner modal stops propagation */}
      <div className="share-modal" onClick={(e) => e.stopPropagation()}>
        <div className="share-modal-header">
          <div className="share-modal-title">
            <img src="/favicon.ico" alt="" className="share-modal-logo" />
            <h3>Share</h3>
          </div>
          <CloseButton onClick={onClose} size={14} />
        </div>

        <p className="share-modal-tagline">{tagline}</p>
        ...
        <div className="share-options">
          {PLATFORMS.map(({ name, key, color }, i) => (
            <a
              ...
              style={{ '--i': i, '--hover-color': color } as React.CSSProperties}
              onClick={(e) => {
                e.preventDefault();
                handleSocialClick(links[key as keyof typeof links] ?? '#');
              }}
            >
              {/* biome-ignore lint/security/noDangerouslySetInnerHtml: static SVG icons from ICONS constant */}
        ...
        <div className="share-link-row">
          <input type="text" value={url} readOnly className="share-link-input" />
          <button
            type="button"
